...
public  function terms(Request $request)
{
    ...
    public function signup(Request $request){
        ...
        $personalID = date('m').date('d').substr($request['mobile_no'], -4);
       // dd($personalID);
 if($request->file('profile_pic')){
        $appuser = Appuser::insertGetId(["email"=>$request['email'],"locality"=>$request['locality'],"home_town"=>$request['home_town'],"profile_pic"=>$profle_pic,"name"=>$request['name'],"country_code"=>$request['country_code'],"mobile_no"=>$request['mobile_no'],"country"=>$request['country'],"address"=>$request['city'],
        "password"=>$password,"device_type"=>$request['device_type'],"device_token"=>$request['device_token'],"latitude"=>$request['latitude'],"longitude"=>$request['longitude'],"accesstoken"=>$accesstoken,"otp"=>$otp,'personal_id'=>$personalID,'postal_code'=>$request['postal_code']]);
        ...
        $appuser = Appuser::insertGetId(["email"=>$request['email'],"locality"=>$request['locality'],"home_town"=>$request['home_town'],"name"=>$request['name'],"country_code"=>$request['country_code'],"mobile_no"=>$request['mobile_no'],"country"=>$request['country'],"address"=>$request['city'],
        "password"=>$password,"device_type"=>$request['device_type'],"device_token"=>$request['device_token'],"latitude"=>$request['latitude'],"longitude"=>$request['longitude'],"accesstoken"=>$accesstoken,"otp"=>$otp,'personal_id'=>$personalID,'postal_code'=>$request['postal_code']]);
    ...
    public function login(Request $request){
        ...
        if($user_details){
            ...
            if(Hash::check($request["password"],$user_details->password)){
                ...
                $user_details = Appuser::where('id',$user_details->id)->first();
                if($user_details->verified==0){
                    $otp = rand(999,9999);
                    $user_details->otp = $otp;
                    $this->sendsms("emergency taxi verification OTP ".$otp,$user_details->country_code.$user_details->mobile_no);
                }
                ...
                $userdetails =  Appuser::where(["id"=>$user_details->id])->first();
               
                

                return response()->json(["message"=>"Driver logged in successfully","userdetails"=>$userdetails]);
            }else{
