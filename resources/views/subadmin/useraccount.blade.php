            ...
            <div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
               <div class="modal-dialog">
                  <div class="modal-content model-border">
                     ...
                     <form action="{{ url('subadmin/updatedriver') }}" method="post">
                     <div class="modal-body">
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">      
          <div class="modal-body">
            ...
            <form method="post" action="{{ url('subadmin/send_reason_to_driver') }}">
               {{ csrf_field() }}
               <div class="form-group">
                  <label>Reason</label>
...
function confirm_delete(){
   window.location.href="{{ url('subadmin/deleteuser') }}/"+delete_id;
}

function confirm_block(){
   window.location.href="{{ url('subadmin/blockuser') }}/"+block_id;
}
...
function verify_proof(type,status){
  ...
  if(status==1)
   {
     $.ajax({
        url : "{{ url('subadmin/verify_document') }}/"+type,
        type:"post",
        data : {"driver_id":detaildriver_id,"_token":"{{ csrf_token() }}"},
        dataType:'json',
        success:function(json){
           if(json.verified_user){
              window.location.href = "{{ url('subadmin/user_management') }}/1";
           }else{
                  if(type==1){
...
function submitReason(){
  ...
  if(message.length>0){
    $.ajax({
        url : "{{ url('subadmin/unverify_document') }}",
        type:"POST",
...
function view_details(id,organizationtype,vehicletype,vehicle_number,vehicle_image,id_proof,driving_license,profile_pic,vehicle_registration_certificate,taximeter_certificate,id_proof_verified,driving_license_verified,vehicle_registration_certification_verified,taximeter_certificate_verified){
   ...
   if(id_proof_verified==0){
    var str = `<button class="btn btn-success btn-xs" onclick="verify_proof(1,1)" type="button" >verify</button> &nbsp;<button class="btn btn-danger btn-xs" onclick="verify_proof(1,0)"  type="button" >un-verify</button>`;
    $(".id_proof").html(str);
   }else if(id_proof_verified==1){
    var str = `<span class="text-success">verified</span>`;
    $(".id_proof").html(str);
   }else{
    var str = `<span class="text-danger">un-verified</span>`;
  ...
  if(driving_license_verified==0){
    var str = `<button class="btn btn-success btn-xs" onclick="verify_proof(2,1)"  type="button" >verify</button> &nbsp;<button class="btn btn-danger btn-xs" onclick="verify_proof(2,0)"  type="button" >un-verify</button>`;
    $(".driving_license").html(str);
   }else if(driving_license_verified==1){
    var str = `<span class="text-success">verified</span>`;
    $(".driving_license").html(str);
   }else{
    var str = `<span class="text-danger">un-verified</span>`;
   ...
   if(vehicle_registration_certification_verified==0){
    var str = `<button class="btn btn-success btn-xs" onclick="verify_proof(3,1)" type="button" >verify</button> &nbsp;<button class="btn btn-danger btn-xs" onclick="verify_proof(3,0)" type="button" >un-verify</button>`;
    $(".vehicle_registration").html(str);
   }else if(vehicle_registration_certification_verified==1){
    var str = `<span class="text-success">verified</span>`;
    $(".vehicle_registration").html(str);
   }else{
    var str = `<span class="text-danger">un-verified</span>`;
   ...
   if(taximeter_certificate_verified==0){
    var str = `<button class="btn btn-success btn-xs" onclick="verify_proof(4,1)" type="button" >verify</button> &nbsp;<button class="btn btn-danger btn-xs" onclick="verify_proof(4,0)" type="button" >un-verify</button>`;
    $(".teximeter").html(str);
   }else if(taximeter_certificate_verified==1){
    var str = `<span class="text-success">verified</span>`;
    $(".teximeter").html(str);
   }else{
    var str = `<span class="text-danger">un-verified</span>`;
...
$(".verified_status").change(function(){
   window.location.href="{{ url('subadmin/user_management') }}/"+$(this).val();
});
...
function confirm_verify(){
   window.location.href="{{ url('subadmin/verify_driver') }}/"+verify_id;
}
...
<script>
  ...
  $('ul.tabs li').on('click',function(){
   ...
   $('.service_type').change(function(){
      ...
      $.ajax({
         url:"{{ url('subadmin/set_user_service_type') }}",
         type:'post',
