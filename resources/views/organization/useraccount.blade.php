				  ...
				  <tbody>
					 @foreach($verifiedusers as $i => $user)
					   <tr>
						  ...
						  <td>@if($user['servicetype']) {{ $user['servicetype']['name'] }} @else 
						  ...
						  @endif
						  @if($user['my_subscription']['subscription_id']==12)
						 </br> <span class="text-success plantype" data-plantype="{{$user['my_subscription']['subscription_id'].'-'.$user['my_subscription']['driver_id'].'-'.$user['my_subscription']['duration'].'-'.$user['my_subscription']['type']}}" style="cursor:pointer;">Extend Days </span>
						@endif
						  ...
						  <td>{{ \App\Search::where(['driver_id' => $user['id']])->get()->count() }}</td>
					  ...
					  <table id="mytable1" class="table table-striped table-bordered">
						 ...
						 <tbody>
							...
							<tr>
							   ...
							   <td>{{ date('Y-m-d',strtotime($user['created_at'])) }}</td>
							   <td class="mwpx-170">{{ $user['address'] }} </td>
							   <td class="inactive">@if($user['subscribed'] ==1)<span class="text-success">Subscribed</span> @else <span class="text-danger"> Not subscribed </span>@endif</td>
							   <td class="inactive"> 
...
function changeUrl(type){
  window.location.href = "{{url('organization/user_management')}}"+"/"+type;
}


function confirm_delete(){
   window.location.href="{{ url('organization/deleteuser') }}/"+delete_id;
}

function confirm_block(){
   window.location.href="{{ url('organization/blockuser') }}/"+block_id;
}
...
function verify_proof(type,status){
  ...
  if(status==1)
   {
     $.ajax({
        url : "{{ url('organization/verify_document') }}/"+type,
        type:"post",
        data : {"driver_id":detaildriver_id,"_token":"{{ csrf_token() }}"},
        dataType:'json',
        success:function(json){
           if(json.verified_user){
              window.location.href = "{{ url('organization/user_management') }}/1";
           }else{
...
function submitReason(){
  ...
  if(message.length>0){
    $.ajax({
        url : "{{ url('organization/unverify_document') }}",
        type:"POST",
...
function view_details(id){
   ...
   $.ajax({
      url : '{{ url('organization/userdetails') }}/'+detaildriver_id,
      type:'get',
      dataType:'json',
      success:function(json){
        json = json.appuser;
         ...
         $("#vehicle_image_tag").attr('href',"{{ asset('public/files') }}/"+json.vehicle_image);
         $("#id_proof_tag").attr('href',"{{ asset('public/files') }}/"+json.id_proof);
         $("#driving_license_tag").attr('href',"{{ asset('public/files') }}/"+json.driving_license);
         $("#vehicle_registration_tag").attr('href',"{{ asset('public/files') }}/"+json.vehicle_registration_certificate);
         $("#teximeter_tag").attr('href',"{{ asset('public/files') }}/"+json.taximeter_certificate);
         

         $(".vehicle_image_preview_tag").attr('href',"{{ asset('public/files') }}/"+json.vehicle_image);
         $(".id_proof_preview_tag").attr('href',"{{ asset('public/files') }}/"+json.id_proof);
         $(".driving_license_preview_tag").attr('href',"{{ asset('public/files') }}/"+json.driving_license);
         $(".vehicle_registration_preview_tag").attr('href',"{{ asset('public/files') }}/"+json.vehicle_registration_certificate);
         $(".teximeter_preview_tag").attr('href',"{{ asset('public/files') }}/"+json.taximeter_certificate);
...
$(".verified_status").change(function(){
   window.location.href="{{ url('organization/user_management') }}/"+$(this).val();
});
...
function confirm_verify(){
   window.location.href="{{ url('organization/verify_driver') }}/"+verify_id;
}
...
// })
   ...
   $('.service_type').change(function(){
      ...
      if(service_type){
       // alert(0);
        $.ajax({
           url:"{{ url('organization/set_user_service_type') }}",
           type:'post',
